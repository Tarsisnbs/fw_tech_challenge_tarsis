...
static void uart1_safe_send(const char *str) { 
    if (uart1_mutex) {
        ...
        // If the mutex doesn't exist yet (right at the beginning), send it directly.
        hal_uart1_send_str(str);
    }
}
/* --- INTERRUPT HANDLERS --- */
...
void pb0_falling_callback(void) {
    if (xSHELLTaskHandle != NULL) {
        BaseType_t woken = pdFALSE;
        vTaskNotifyGiveFromISR(xSHELLTaskHandle, &woken);
        portYIELD_FROM_ISR(woken);
    }
}
...
void fw_init(void) {
    ...
    // Secure priority of IRQ_USART2 configuration for FreeRTOS (Prevents Hard Fault)
